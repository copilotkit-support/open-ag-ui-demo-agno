...
import numpy as np
import pandas as pd
from prompts import insights_prompt, system_prompt
...
async def simultion(step_input):
    ...
    step_input.additional_data["be_stock_data"] = data["Close"]
    step_input.additional_data["be_arguments"] = arguments
    index = len(step_input.additional_data["tool_logs"]) - 1
    step_input.additional_data["emit_event"](
        StateDeltaEvent(
            type=EventType.STATE_DELTA,
...
async def cash_allocation(step_input):
    ...
    args = step_input.additional_data["be_arguments"]
    ...
    if step_input.additional_data["available_cash"] is not None:
        total_cash = step_input.additional_data["available_cash"]
    else:
...
async def gather_insights(step_input):
    ...
    if response.choices[0].finish_reason == "tool_calls":
        ...
        step_input.additional_data["insights"] = {}
    index = len(step_input.additional_data["tool_logs"]) - 1
    step_input.additional_data["emit_event"](
        StateDeltaEvent(
            type=EventType.STATE_DELTA,
